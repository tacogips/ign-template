...
github_repository="@ign-var:GITHUB_REPOSITORY=user/repo@"
...
release_tag="${1:-}"
tap_cask_file="${2:-$repo_root/@ign-var:HOMEBREW_TAP_DIR=../homebrew-tap@/Casks/$artifact_name.rb}"
...
if ! gh release view "$release_tag" --repo "$github_repository" >/dev/null 2>&1; then
  gh release create "$release_tag" \
    ...
    --title "@ign-var:PROJECT_NAME={current_dir}@ $release_tag" \
    --notes "$release_notes"
fi
...
printf '  brew tap @ign-var:HOMEBREW_TAP=user/tap@\n'
printf '  brew install --cask @ign-var:HOMEBREW_CASK_TOKEN={current_dir}@\n'
