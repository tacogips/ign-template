...
brew install --cask @ign-var:HOMEBREW_CASK_TOKEN={current_dir}@
```
...
swift run @ign-var:EXECUTABLE_NAME={current_dir}@ --version | tail -n 1 | grep -Fx "$version"
```
...
```bash
version="$(tr -d '[:space:]' < VERSION)"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/@ign-var:HOMEBREW_CASK_TOKEN={current_dir}@-${version}-darwin-arm64.dmg"
/Applications/Xcode.app/Contents/Developer/usr/bin/stapler validate "dist/homebrew-cask/@ign-var:HOMEBREW_CASK_TOKEN={current_dir}@-${version}-darwin-x64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/@ign-var:HOMEBREW_CASK_TOKEN={current_dir}@-${version}-darwin-arm64.dmg"
spctl --assess --type open --context context:primary-signature --verbose=4 "dist/homebrew-cask/@ign-var:HOMEBREW_CASK_TOKEN={current_dir}@-${version}-darwin-x64.dmg"
```
...
DMGs to `@ign-var:GITHUB_REPOSITORY=user/repo@`, and renders
`@ign-var:HOMEBREW_TAP_DIR=../homebrew-tap@/Casks/@ign-var:HOMEBREW_CASK_TOKEN={current_dir}@.rb`.

After reviewing the rendered tap Cask:

```bash
cd @ign-var:HOMEBREW_TAP_DIR=../homebrew-tap@
git add Casks/@ign-var:HOMEBREW_CASK_TOKEN={current_dir}@.rb README.md
git diff --staged --stat
git commit -m "chore: release @ign-var:HOMEBREW_CASK_TOKEN={current_dir}@ <version>"
