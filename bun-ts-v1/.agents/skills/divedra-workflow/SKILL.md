---
name: divedra-workflow
description: Use when creating, modifying, reviewing, or validating divedra workflow bundles. Applies to step-addressed workflow.json authoring, node payload files, prompt files, built-in node add-ons, cross-workflow transitions, manager/worker routing, and portable workflows under a workflow root.
metadata:
  short-description: Create divedra workflow bundles
---

# Divedra Workflow

Use this skill to author portable divedra workflow bundles that validate against the current implementation.

## Required Workflow

1. Determine the workflow root and workflow name. If unspecified, use a local workflow root such as `.divedra/workflows` or `examples` only when that matches the repository convention.
...
5. Validate with the available divedra command, usually `bun run src/main.ts workflow validate <workflow-name> --workflow-root <workflow-root>` inside the divedra repo, or `divedra workflow validate <workflow-name> --workflow-root <workflow-root>` when installed.
...
```json
{
  ...
  "prompts": {
    "divedraPromptTemplate": "Coordinate {{workflowId}} and route work to the worker.",
...
- Valid `executionBackend` values are `codex-agent`, `official/openai-sdk`, and `official/anthropic-sdk`.
...
Current built-ins include `divedra/chat-reply-worker`, `divedra/codex-worker`, `divedra/x-gateway-read`, `divedra/x-gateway`, `divedra/mail-gateway-read`, `divedra/mail-gateway`, `divedra/git-commit`, and `divedra/git-push`, all version `1`.

## External Portability

When creating workflows for users outside the divedra repository:
...
- If the divedra CLI is not available, still produce the bundle but state that validation could not be run.
